...
function Signup({setCurrentUser, setUserData}) {
   ...
   // Function handle signing up with new user information. After signing up, setCurrentUser to new user's information, then add that new user object to userData array, and setUserData to that new array.
            ...
            if (resp.ok) {
               ...
               resp.json()
                  .then(error => {
                     ...
                     // With the remaining possible errors, render them so username errors show before password errors
